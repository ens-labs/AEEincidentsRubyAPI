	...
	def all_averias
		...
		pueblos.each do |value|
			...
			if data[:get_breakdowns_by_town_or_city_response][:return].kind_of?(Array)
				...
				hash_pueblo["#{data[:get_breakdowns_by_town_or_city_response][:return][averias][:r1_town_or_city]}"] = array_averias
			else
				hash_averia = Hash.new
				...
				hash_pueblo["#{data[:get_breakdowns_by_town_or_city_response][:return][:r1_town_or_city]}"] = hash_averia
			end
		end
		return hash_pueblo.to_json
	end
